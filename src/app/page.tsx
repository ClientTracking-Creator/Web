...
function SubscriptionScreen() {
  ...
  return (
    <>
      ...
      {payment && proofOpen && <Modal title={t("bankTransactionPhoto")} close={() => setProofOpen(false)}><div className="space-y-4"><Label text={t("bankTransactionPhoto")}><Field type="file" accept="image/*" required onChange={(e) => { const file = e.target.files?.[0] || null; setProofFile(file); setProofPreview(file ? URL.createObjectURL(file) : ""); }} /></Label>{proofPreview && <ImageWithFallback src={proofPreview} alt="" className="h-48 w-full rounded-lg object-contain" />}<Button disabled={sendingRequest || requestSent} onClick={() => { if (!proofFile) { setProofAlert(true); return; } runPaymentRequest(async () => { const proofImageUri = userProfile?.uid ? await uploadImageToCloudinary(proofFile, userProfile.uid, "payment_proofs") : ""; await addPaymentRequest({ id: payment.qr.md5, uid: userProfile?.uid || "", email: userProfile?.email || "", planTitle: payment.plan.title, amount: payment.plan.amount, months: payment.plan.months, md5: payment.qr.md5, status: "pending", createdAt: new Date().toISOString(), proofImageUri, error: paymentError }); setRequestSent(true); setProofOpen(false); }); }} className="w-full">{sendingRequest ? t("uploading") : t("send")}</Button></div></Modal>}
