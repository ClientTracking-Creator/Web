  ...
  return (
    <>
      ...
      <div className="space-y-5 px-5 lg:px-8">
        ...
        <div className="grid gap-3 md:grid-cols-2">{plans.map((plan) => <button key={plan.title} onClick={() => { setPayment({ plan, qr: generatePaymentQR(plan.amount, "USD") }); setSuccess(false); setPaymentError(""); setRequestSent(false); setProofFile(null); setProofPreview(""); setProofOpen(false); }} className="flex min-w-0 items-center justify-between gap-3 rounded-xl border border-[#3a3a3c] bg-[#1e1e1e] p-5 text-left"><div className="min-w-0"><h3 className="break-anywhere text-lg font-black">{plan.title}</h3><p className="text-2xl font-black text-[#ccff00]">${plan.amount}</p></div><span className="break-anywhere shrink-0 rounded-lg bg-[#ccff00] px-3 py-2 text-center text-sm font-black leading-tight text-black">{t("subscribeNow")}</span></button>)}</div>
        <Card className="flex gap-3 text-sm text-[#a0a0a5]"><AlertCircle size={20} /> {t("subscriptionInfo")}</Card>
      </div>
      {payment && <Modal title={success ? t("paymentSuccess") : t("paymentTitle")} close={() => setPayment(null)}>{success ? <div className="text-center"><Check className="mx-auto mb-4 text-[#ccff00]" size={80} /><p className="mb-4 text-[#a0a0a5]">{t("subscriptionUpdated")}</p><Button onClick={() => setPayment(null)} className="w-full">{t("done")}</Button></div> : <div className="flex flex-col items-center gap-4"><p className={timeLeft < 60 ? "text-[#ff453a]" : "text-[#a0a0a5]"}>{t("sessionExpires")}{Math.floor(timeLeft / 60)}:{String(timeLeft % 60).padStart(2, "0")}</p><KHQRCard qrString={payment.qr.qrString} amount={payment.plan.amount} /><p className="text-[#a0a0a5]">{timeLeft > 0 ? t("paymentPending") : t("sessionExpired")}</p>{paymentError && <p className="text-center text-sm text-[#ff453a]">{paymentError}</p>}<Button disabled={checkingPayment} onClick={() => runPaymentCheck(() => verifyPayment(payment, true))} className="w-full">{checkingPayment ? t("checkingPayment") : t("checkPayment")}</Button><Button variant="ghost" disabled={requestSent} onClick={() => setProofOpen(true)} className="w-full">{requestSent ? t("paymentRequestSent") : t("iHavePaid")}</Button><p className="text-lg font-bold">{t("scanToPay")}</p></div>}</Modal>}
      {payment && proofOpen && <Modal title={t("bankTransactionPhoto")} close={() => setProofOpen(false)}><div className="space-y-4"><Label text={t("bankTransactionPhoto")}><Field type="file" accept="image/*" required onChange={(e) => { const file = e.target.files?.[0] || null; setProofFile(file); setProofPreview(file ? URL.createObjectURL(file) : ""); }} /></Label>{proofPreview && <ImageWithFallback src={proofPreview} alt="" className="h-48 w-full rounded-lg object-contain" />}<Button disabled={sendingRequest || requestSent} onClick={() => { if (!proofFile) { setProofAlert(true); return; } runPaymentRequest(async () => { const proofImageUri = userProfile?.uid ? await uploadImageToCloudinary(proofFile, userProfile.uid, "payment_proofs") : ""; await addPaymentRequest({ id: payment.qr.md5, uid: userProfile?.uid || "", email: userProfile?.email || "", planTitle: payment.plan.title, amount: payment.plan.amount, months: payment.plan.months, md5: payment.qr.md5, status: "pending", createdAt: new Date().toISOString(), proofImageUri, error: paymentError }); setRequestSent(true); setProofOpen(false); setPayment(null); }); }} className="w-full">{sendingRequest ? t("uploading") : t("send")}</Button></div></Modal>}
      {proofAlert && <Modal title={t("bankTransactionPhoto")} close={() => setProofAlert(false)}><div className="text-center"><AlertCircle className="mx-auto mb-4 text-[#ff9800]" size={56} /><p className="mb-5 text-[#f5f5f7]">{t("pleaseAddBankTransactionPhoto")}</p><Button onClick={() => setProofAlert(false)} className="w-full">{t("done")}</Button></div></Modal>}
    </>
  );
}
...
function PaymentRequestRow({ request, approve, reject }: { request: PaymentRequest; approve: (request: PaymentRequest) => Promise<void>; reject: (request: PaymentRequest) => Promise<void> }) {
  ...
  return (
    <div className="rounded-lg border border-[#3a3a3c] bg-[#121212] p-3">
      ...
      {request.proofImageUri && <a href={request.proofImageUri} target="_blank" rel="noreferrer" className="mb-3 block overflow-hidden rounded-lg border border-[#3a3a3c] bg-black"><ImageWithFallback src={request.proofImageUri} alt="" className="h-44 w-full object-contain" /></a>}
      <div className="grid grid-cols-2 gap-2">
